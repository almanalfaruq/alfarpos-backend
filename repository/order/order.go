...
func (repo *OrderRepository) FindByFilter(status []int32, invoice, startDate, endDate, sort string, limit, page int32) ([]orderentity.Order, error) {
	...
	if startDate != "" || endDate != "" {
		if startDate == endDate {
			whereClauses = append(whereClauses, fmt.Sprintf("timezone('UTC', orders.created_at) = '%s'", startDate))
		} else {
			if startDate != "" {
				whereClauses = append(whereClauses, fmt.Sprintf("timezone('UTC', orders.created_at) >= '%s'", startDate))
			}
			if endDate != "" {
				whereClauses = append(whereClauses, fmt.Sprintf("timezone('UTC', orders.created_at) <= '%s'", endDate))
			}
		}
	}
	sortSql := "orders.id"
